...
public class ExpressionModule
{
    ...
    {
        Logger.Log("ExpressionModule::ExpressionModule(bricks)");
        if(isBioBricksSequenceValid(bricks))
        {
    ...
    {
        Logger.Log("ExpressionModule::ExpressionModule("+m+")");
        _name = m._name;
        _bioBricks = new LinkedList<BioBrick>();
    ...
    {
        Logger.Log("ExpressionModule::checkOperon("+bricks+")");
        bool b = false;
    ...
    public static bool isBioBricksSequenceValid(LinkedList<BioBrick> bioBricksToCheck)
    {
        if (bioBricksToCheck == null) {
            Logger.Log("ExpressionModule::isBioBricksSequenceValid failed (bioBricksToCheck == null)");
            return false;
        }
        ...
        //PROMOTER
        if (bricks.Count == 0 || bricks.First.Value == null) {
            Logger.Log("ExpressionModule::isBioBricksSequenceValid failed: no promoter");
            return false;
        }
        if (checkPromoter(bricks.First.Value) == false) {
            Logger.Log("ExpressionModule::isBioBricksSequenceValid failed (checkPromoter(bricks.First.Value) == false)");
            return false;
        }
        bricks.RemoveFirst();
        ...
        //RBS & ORF
        if (bricks.Count == 0 || bricks.First.Value == null) {
            Logger.Log("ExpressionModule::isBioBricksSequenceValid failed: no RBS/ORF/Terminator");
            return false;
        }
        if (checkOperon(bricks) == false) {
            Logger.Log("ExpressionModule::isBioBricksSequenceValid failed (checkOperon(bricks) == false)");
            return false;
        }        
        ...
        if (b1 || b2 || b3) {
            if (b1) Logger.Log("ExpressionModule::isBioBricksSequenceValid failed: no terminator");
            if (b2) Logger.Log("ExpressionModule::isBioBricksSequenceValid failed: terminator is null");
            if (b3) Logger.Log("ExpressionModule::isBioBricksSequenceValid failed: not a terminator");
            return false;
        }
        bricks.RemoveFirst();
        if (bricks.Count != 0) {
    ...
    public bool isValid()
    {
        Logger.Log("ExpressionModule::checkModuleValidity("+this.ToString()+")");
